 ...
 * CannotPlayAi. Only one sub-ability shape is understood so far: Blight
 * Titan's "mill yourself two, then incubate X". Every other Incubate keeps
 * CannotPlayAi's refusal. That covers top-level SP$/AB$ Incubate (canPlay is
 * inherited) and every other Incubate hung under another effect (chkDrawback
 * falls through to super). Assimilate Essence, Excise the Imperfect,
 * Merciless Repurposing, Searing Barb, Sunder the Gateway, Sunfall, Tangled
 * Skyline and Traumatic Revelation therefore behave exactly as before.
 */
